...
func (tq *TodoQuery) QueryForm() *FormQuery {
	query := (&FormClient{config: tq.config}).Query()
...
func (tq *TodoQuery) FirstID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (tq *TodoQuery) FirstIDX(ctx context.Context) int32 {
...
func (tq *TodoQuery) OnlyID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (tq *TodoQuery) OnlyIDX(ctx context.Context) int32 {
...
func (tq *TodoQuery) IDs(ctx context.Context) ([]int32, error) {
	var ids []int32
...
func (tq *TodoQuery) IDsX(ctx context.Context) []int32 {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TodoQuery) WithForm(opts ...func(*FormQuery)) *TodoQuery {
	query := (&FormClient{config: tq.config}).Query()
...
func (tq *TodoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Todo, error) {
	...
	if query := tq.withForm; query != nil {
		if err := tq.loadForm(ctx, query, nodes,
			func(n *Todo) { n.Edges.Form = []*Form{} },
			func(n *Todo, e *Form) { n.Edges.Form = append(n.Edges.Form, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TodoQuery) loadForm(ctx context.Context, query *FormQuery, nodes []*Todo, init func(*Todo), assign func(*Todo, *Form)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int32]*Todo)
	for i := range nodes {
	...
	query.Where(predicate.Form(func(s *sql.Selector) {
		s.Where(sql.InValues(todo.FormColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "todo_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
